...
results <- replicate(1000, {
  ...
  median_score <- median(mydata$evolutionary_conservation_score[selected_indices])
  c(median_score, sd_score)
})

# Extract median values
medians <- results[seq(1, length(results), 2)]
# Compute percentiles
percentiles <- quantile(medians, c(0.025, 0.975))
...
# Plot histogram with median and percentile lines
hist(medians, breaks=50, xlab="Median conservation score", main="Distribution of median conservation score")
abline(v=mean(medians), col="red", lwd=2)
