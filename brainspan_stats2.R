##This code reads in RBP expression data from Brainspan (shown is code for BRUNOL4/CELF4), finds rows with "frontal" in the brain_region column (for analysis of frontal cortex samples). It checks normality using Shapiro-Wilk Test. It then calculates the median and interquartile range for the rows before row 159 ("early" fetal data) and after 160 but before row 239 ("late" fetal data), and performs a Mann-Whitney U test for comparison. It also counts the number of samples in the early fetal group and the late fetal group. It prints all of the above mentioned values as well.

data <- read.csv("/home/a/aangeles/Downloads/CELF4_data.csv")
head(data)

# use grep to find rows with "frontal" (i.e. grepping frontal cortex data from BrainSpan dataset)
